...
class TagsApi {
  ...
  Future<List<ReadTag>> tagsGet({ bool orderByMostUsed }) async {
    ...
    if(orderByMostUsed != null) {
      queryParams.addAll(_convertParametersForCollectionFormat("", "orderByMostUsed", orderByMostUsed));
    }
    ...
    if(response.statusCode >= 400) {
      ...
      return 
        (apiClient.deserialize(response.body, 'List<ReadTag>') as List).map((item) => item as ReadTag).toList();
    } else {
      return null;
    }
  }
