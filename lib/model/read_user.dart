...
class ReadUser {
  ...
  ReadUser.fromJson(Map<String, dynamic> json) {
    if (json == null) return;
    userId = json['userId'];
    nameIdentifier = json['nameIdentifier'];
    email = json['email'];
  ...
  static List<ReadUser> listFromJson(List<dynamic> json) {
    return json == null
        ? new List<ReadUser>()
        : json.map((value) => new ReadUser.fromJson(value)).toList();
  }

  static Map<String, ReadUser> mapFromJson(
      Map<String, Map<String, dynamic>> json) {
    var map = new Map<String, ReadUser>();
    if (json != null && json.length > 0) {
      json.forEach((String key, Map<String, dynamic> value) =>
          map[key] = new ReadUser.fromJson(value));
    }
    return map;
  }
}
